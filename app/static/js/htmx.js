// htmx 4.0.0-beta4
// Vendored verbatim from the htmx `force-named-arg` branch (src/htmx.js).
// Includes: process(elt, {force}), deep __cleanup re-wiring, the
// htmx:before:morph:attr extension hook, and hx-on bind-once dedup.
// Re-vendor from upstream to update; do not hand-edit.
var htmx = (() => {
    ...
    class Htmx {
        ...
        // options.force: unbind and re-wire the whole subtree, picking up hx attributes
        // that were mutated in place (process() alone only wires elements not yet wired).
        process(elt, {force} = {}) {
            if (!elt) return;
            ...
            // Reset elt and every already-powered descendant so the init passes below
            // rebind them from current attributes. Surgical (keeps request-queue and
            // indicator/disabled ref-counts) so forcing during an in-flight request does
            // not leave a stuck indicator.
            if (force) {
                this.__cleanup(elt);
            }
        ...
        // Tears down listeners/timers/observers and resets init flags.
        // Keeps in-flight request state so cleanup mid-request won't stick an indicator.
        // Fires cleanup events so extensions (e.g. hx-live) drop and re-register their state.
        __cleanup(elt) {
            for (let el of [elt, ...(elt.querySelectorAll?.('[data-htmx-powered]') || [])]) {
                if (!el._htmx) continue;
                this.__trigger(el, "htmx:before:cleanup");
                for (let spec of el._htmx.triggerSpecs || []) {
                ...
                for (let info of el._htmx.listeners || []) {
                    info.fromElt.removeEventListener(info.eventName, info.handler, info);
                }
                el._htmx.listeners = [];
                el._htmx.triggerSpecs = [];
                delete el._htmx.initialized;
                delete el._htmx.onInitialized;
                delete el._htmx.boosted;
                delete el._htmx.eventHandler;
                el.removeAttribute('data-htmx-powered');
                this.__trigger(el, "htmx:after:cleanup");
            }
        }
