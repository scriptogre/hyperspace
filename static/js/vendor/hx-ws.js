(() => {
    ...
    function getOrCreateConnection(url, element) {
        ...
        if (!createWebSocket(connection, element)) return null;
    ...
    function createWebSocket(connection, element) {
        ...
        // Native WebSocket construction passthrough. Hook may mutate detail.url
        // (e.g., append fresh auth token) or detail.protocols (subprotocol negotiation).
        let elt = element || findConnectedElement(connection.url);
        connection.cancelled = false;
        let detail = {
            connection,
            url: connection.url,
            protocols: connection.config?.protocols,
        };
        if (!api.triggerHtmxEvent(elt, 'htmx:before:ws:connection', detail) || connection.cancelled) {
        ...
        try {
            connection.socket = new WebSocket(detail.url, detail.protocols);
            let ac = new AbortController();
            connection.abortController = ac;
            let opts = { signal: ac.signal };

            connection.socket.addEventListener('open', () => {
                let openElt = findConnectedElement(connection.url);
                if (openElt) {
                    api.triggerHtmxEvent(openElt, 'htmx:after:ws:connection', {connection});
                } else {
            ...
            connection.socket.addEventListener('close', (event) => {
                ...
                let closeElt = findConnectedElement(connection.url);
                if (closeElt) api.triggerHtmxEvent(closeElt, 'htmx:ws:close', {
                    connection, reason: 'closed', code: event.code, event
                });

                if (!connections.has(connection.url)) return;

                let config = connection.config;
                if (config.pauseOnBackground && document.hidden) return;

                if (config.reconnect && findConnectedElement(connection.url)) {
            ...
            connection.socket.addEventListener('error', (error) => {
                let errElt = findConnectedElement(connection.url);
                if (errElt) api.triggerHtmxEvent(errElt, 'htmx:ws:error', { url: connection.url, error });
            }, opts);
    ...
    function scheduleReconnect(connection) {
        ...
        if (!findConnectedElement(connection.url)) {
            cleanupOrphanedConnection(connection.url, connection);
            return;
        }

        connection.timer = setTimeout(() => {
            if (!findConnectedElement(connection.url) || !createWebSocket(connection)) {
                cleanupOrphanedConnection(connection.url, connection);
            }
        }, delay);
    }
