...
test('full interactive walkthrough', async ({ page }) => {
  ...
  await expect(page.locator('.iso-block')).toHaveCount(1, { timeout: 5000 });
  ...
  await expect(page.locator('.iso-block')).toHaveCount(2, { timeout: 5000 });
  await page.click('.iso-cell[data-x="6"][data-y="5"]');
  await expect(page.locator('.iso-block')).toHaveCount(3, { timeout: 5000 });
  await page.click('.iso-cell[data-x="4"][data-y="7"]');
  await expect(page.locator('.iso-block')).toHaveCount(4, { timeout: 5000 });
  await page.waitForTimeout(500);
  await shot(page, '03-four-blocks');

  // 5. Verify sidebar lists all blocks
  await expect(page.locator('aside')).toContainText('Blocks · 4');
  ...
  const blockEntry = page.locator('aside .group', { hasText: '(3,2)' });
  ...
  await expect(page.locator('aside')).toContainText('Blocks · 3');
